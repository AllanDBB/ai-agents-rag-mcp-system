...
LLM agentes & claude-haiku-4-5 & Bajo costo para agentes especializados \\
Embeddings & all-MiniLM-L6-v2 (ONNX) & Local, sin costo, multilingüe, 384 dims \\
Vector DB & ChromaDB & Persistente local, sin infraestructura \\
...
Web Search & Tavily API & Plan gratuito disponible \\
...
\textbf{Configuración A — Chunk fijo:} fragmentos de 512 caracteres con solapamiento de 64. Simple y predecible. Produce 1{,}238 chunks de 51 documentos.
...
\midrule
A (512 fijo) & 0.405 & 419 & 1{,}238 \\
B (1024 semántico) & 0.454 & 775 & 643 \\
\bottomrule
\end{tabular}
\end{table}

La configuración A produce menor distancia coseno promedio (mayor similitud), lo que indica recuperación más precisa a nivel de fragmento. La configuración B, al producir chunks más largos, incluye más contexto por fragmento pero con menor precisión semántica. Para preguntas concretas y factuales, A es superior; para preguntas que requieren comprensión de párrafos completos, B puede ser más adecuada.
...
La comparación de configuraciones RAG evidencia un trade-off entre precisión (Config A, chunks cortos) y contexto (Config B, chunks largos): para preguntas específicas y factuales, la Config A produce recuperación más precisa; para preguntas que requieren comprensión de conceptos completos, la Config B puede ser preferible.
...
Como trabajo futuro se recomienda: incorporar un modelo de embeddings multilingüe especializado en español, ampliar el corpus con todos los apuntes del curso, e implementar un mecanismo de reconexión automática al MCP Server.
